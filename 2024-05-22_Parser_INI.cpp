...
int main()
{
    ...
    std::string fname = "";
    std::cout << "To find a file, type in the file name (hint: setup.ini). " << std::endl;
    std::cin >> fname;
    std::ifstream is(fname);
    try
    {
        if (!is.is_open()) throw std::exception("File not exist");
    }
    ...
    inipp::ini_parser<char> parser;
    parser.parse(is);
    std::string value = "";
    inipp::get_value(parser.sections["General Settings"], "ForceInstallRtk", value);
    try
    {
        if (value == "") throw std::exception("Value is empty");
        else std::cout << "General Settings ForceInstallRtk: " << value << std::endl;
    }
    catch (const std::exception& ex)
    {
        std::cout << "General Settings ForceInstallRtk значение отсутствует" << std::endl;
    }

    inipp::get_value(parser.sections["General Settings"], "ExPgPath_Vista", value);
    try
    {
        
        if (value == "") throw std::exception("Value is empty");
        else std::cout << "General Settings ExPgPath_Vista: " << value << std::endl;
    }
    catch (const std::exception& ex)
    {
        std::cout << "General Settings ExPgPath_Vista значение отсутствует" << std::endl;
    }
...
Содержание
1. Введение
INI-файлы — это обычные текстовые файлы, которые можно редактировать и просматривать при помощи любого текстового редактора. INI-файлы приобрели популярность в Windows, хотя могут использоваться в любой ОС.
